...
import { EMAIL_CODE_MINUTES } from "@/domain/staff";
...
/**
 * How a sign-in code is kept in `app_verification`: an HMAC keyed with
 * BETTER_AUTH_SECRET, never the code itself.
 *
 * Better Auth's own `storeOTP: "hashed"` is a bare SHA-256, and a six-digit code
 * has only a million values, so anyone holding a copy of the table (a backup,
 * the Supabase service key, a psql session) could reverse every live code in
 * under a second. Keying the hash with the secret, which lives in the
 * deployment's environment and never in the database, closes that: the table
 * alone gives nothing away. The plugin compares in constant time.
 *
 * Read on each call rather than at import, because `next build` loads this
 * module with no environment at all.
 */
function hashSignInCode(code: string): string {
  const secret = process.env.BETTER_AUTH_SECRET;
  if (!secret) throw new Error("BETTER_AUTH_SECRET is not set; refusing to issue a sign-in code.");
  return createHmac("sha256", secret).update(`blackbook-sign-in:${code}`).digest("base64url");
}
...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    emailOTP({
      ...
      disableSignUp: true,
      /*
       * Never the code itself. A hash cannot be reversed to resend the live
       * code, so pressing "send" again mints a fresh one, which is the safer
       * behaviour anyway.
       */
      storeOTP: { hash: async (code: string) => hashSignInCode(code) },
